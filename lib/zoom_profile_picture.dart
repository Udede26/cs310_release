    ...
    ref.child("users").child(prefs.getString('uid')).once().then((DataSnapshot data) async {
      ...
      values["follower_list"]["id"].forEach((key,value) {
        if(!results.contains(value))
          setState(() {
            results.add(value);
            ref.child("users").child(value).once().then((DataSnapshot data) async {

              names[value] = data.value["username"];
            });
          });
        print("username");
        print(key);
    ...
    return Scaffold(
      ...
      body:  Column(
        children: <Widget>[
          Container(
            height: 800,
             child: FittedBox(child: Image.network(widget.src),fit:BoxFit.cover,),
            ),
        ],
      ),
    );
